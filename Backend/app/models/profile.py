...
class ProfileModel(db.Model):
    ...
    user_id: orm.Mapped[int] = orm.mapped_column(sql.ForeignKey(UserModel.id), index=True, nullable=False)

    user: orm.Mapped["UserModel"] = orm.relationship(back_populates="profiles")
    layers: orm.Mapped[List["LayerModel"]] = orm.relationship(back_populates="profile", cascade="all, delete")
