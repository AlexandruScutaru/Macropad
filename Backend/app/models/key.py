...
class KeyModel(db.Model):
    ...
    layer_id: orm.Mapped[int] = orm.mapped_column(sql.ForeignKey(LayerModel.id, onupdate="CASCADE", ondelete="CASCADE"), index=True, nullable=False)

    layer: orm.Mapped["LayerModel"] = orm.relationship(back_populates="keys")
    options: orm.Mapped[List["OptionModel"]] = orm.relationship(back_populates="key", cascade="all,delete,delete-orphan")
