...
class LayerModel(db.Model):
    ...
    profile_id: orm.Mapped[int] = orm.mapped_column(sql.ForeignKey(ProfileModel.id), index=True, nullable=False)

    profile: orm.Mapped["ProfileModel"] = orm.relationship(back_populates="layers")
    keys: orm.Mapped[List["KeyModel"]] = orm.relationship(back_populates="layer", cascade="all, delete")
